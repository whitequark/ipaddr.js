(function() {
  ...
  ipaddr.IPv4.lastAddressFromCIDR = function(string) {
    var error, octets;
    try {
      octets = this.broadcastAddressFromCIDR(string).toByteArray();
      octets[3] -= 1;
      return new this(octets);
    } catch (error1) {
      error = error1;
  ...
  ipaddr.IPv4.firstAddressFromCIDR = function(string) {
    var error, octets;
    try {
      octets = this.networkAddressFromCIDR(string).toByteArray();
      octets[3] += 1;
      return new this(octets);
    } catch (error1) {
      error = error1;
  ...
  ipaddr.IPv6.lastAddressFromCIDR = function(string) {
    var cidr, error, i, ipInterface, ipInterfaceOctets, octets, subnetMaskOctets;
    try {
      cidr = this.parseCIDR(string);
      ipInterface = cidr[0];
      ...
      return new this(octets, ipInterface.zoneId);
    } catch (error1) {
      error = error1;
  ...
  ipaddr.IPv6.firstAddressFromCIDR = function(string) {
    var cidr, error, i, ipInterface, ipInterfaceOctets, octets, subnetMaskOctets;
    try {
      cidr = this.parseCIDR(string);
      ipInterface = cidr[0];
  ...
  ipaddr.processCIDR = function(string) {
    var addr, cidr, maskLength;
    cidr = this.parseCIDR(string);
    addr = cidr[0];
    maskLength = cidr[1];
