...
export class IPv4 {
    ...
    static broadcastAddressFromCIDR(string) {
    ...
    static isValidFourPartDecimal(string) {
        if (IPv4.isValid(string) && string.match(/^(0|[1-9]\d*)(\.(0|[1-9]\d*)){3}$/)) {
            return true;
        } else {
            return false;
        }
    ...
    static isValidCIDRFourPartDecimal(string) {
        const match = string.match(/^(.+)\/(\d+)$/);

        if (!IPv4.isValidCIDR(string) || !match) {
            return false;
        }

        return IPv4.isValidFourPartDecimal(match[1]);
    };

    // A utility function to return network address given the IPv4 interface and prefix length in CIDR notation
    static networkAddressFromCIDR(string) {
    ...
    static subnetMaskFromPrefixLength(prefix) {
...
export class IPv6 {
    ...
    static broadcastAddressFromCIDR = function (string) {
        try {
            const cidr = this.parseCIDR(string);
            const ipInterfaceOctets = cidr[0].toByteArray();
            const subnetMaskOctets = this.subnetMaskFromPrefixLength(cidr[1]).toByteArray();
    ...
    static isIPv6 = function (string) {
        return this.parser(string) !== null;
    };

    // Checks to see if string is a valid IPv6 Address
    static isValid = function (string) {
    ...
    static isValidCIDR = function (string) {
    ...
    static networkAddressFromCIDR = function (string) {
        let cidr, i, ipInterfaceOctets, octets, subnetMaskOctets;
    ...
    static parseCIDR = function (string) {
    ...
    static subnetMaskFromPrefixLength = function (prefix) {
        prefix = parseInt(prefix);
...
export const fromByteArray = function (bytes) {
...
export const isValidCIDR = function (string) {
    return IPv6.isValidCIDR(string) || IPv4.isValidCIDR(string);
};
...
// Throws an error if it could not be parsed.
export const parseCIDR = function (string) {
    try {
        return IPv6.parseCIDR(string);
    } catch {
        try {
            return IPv4.parseCIDR(string);
        } catch (e) {
...
export const process = function (string) {
...
export const subnetMatch = function (address, rangeList, defaultName) {
