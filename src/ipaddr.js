...
(function() {
  ...
  ipaddr.subnetMaskFromPrefixLength = function(prefix) {
        ...
        var octets = Array(4).fill(0);
        for (j = 0; j < Math.floor(prefix/8); j++) {
            octets[j] = 255;
        }
  ...
  ipaddr.broadcastAddressFromCIDR = function(string) {
        try {
            var ipInterface = ipaddr.IPv4.parseCIDR(string)[0];
            var subnetMask = this.subnetMaskFromPrefixLength([ipaddr.IPv4.parseCIDR(string)[1]]);
        const octets = [];
        for (let i = 0; i < 4; i++) {
