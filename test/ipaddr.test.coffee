...
module.exports =
  ...
  'lastAddressFromCIDR returns correct IPv4 last address': (test) ->
    test.equal(ipaddr.IPv4.lastAddressFromCIDR("172.0.0.1/24"), "172.0.0.254")
    test.equal(ipaddr.IPv4.lastAddressFromCIDR("172.0.0.1/26"), "172.0.0.62")
    test.done()

  'firstAddressFromCIDR returns correct IPv4 first address': (test) ->
    test.equal(ipaddr.IPv4.firstAddressFromCIDR("172.0.0.5/24"), "172.0.0.1")
    test.equal(ipaddr.IPv4.firstAddressFromCIDR("172.0.0.5/5"), "168.0.0.1")
    test.done()
  ...
  'refuses to retrieve broadcastAddressFromCIDR for IPv6': (test) ->
    test.throws ->
      ipaddr.IPv6.broadcastAddressFromCIDR("ac00::1/24")
    test.done()

  'refuses to retrieve networkAddressFromCIDR for IPv6': (test) ->
    test.throws ->
      ipaddr.IPv6.networkAddressFromCIDR("ac00::1/24")
    test.done()

  'lastAddressFromCIDR returns correct IPv6 last address': (test) ->
    test.equal(ipaddr.IPv6.lastAddressFromCIDR("ac00::1/24"), "ac00:ff:ffff:ffff:ffff:ffff:ffff:ffff")
    test.equal(ipaddr.IPv6.lastAddressFromCIDR("ac00::1/26"), "ac00:3f:ffff:ffff:ffff:ffff:ffff:ffff")
    test.equal(ipaddr.IPv6.lastAddressFromCIDR("ac00::1/122"), "ac00::3f")
    test.equal(ipaddr.IPv6.lastAddressFromCIDR("2600:10e5::af/123"), "2600:10e5::bf")
    test.done()

  'firstAddressFromCIDR returns correct IPv6 first address': (test) ->
    test.equal(ipaddr.IPv6.firstAddressFromCIDR("ac00::1/24"), "ac00::")
    test.equal(ipaddr.IPv6.firstAddressFromCIDR("a800::1/5"), "a800::")
    test.equal(ipaddr.IPv6.firstAddressFromCIDR("ac00::1/122"), "ac00::")
    test.equal(ipaddr.IPv6.firstAddressFromCIDR("2600:10e5::af/123"), "2600:10e5::a0")
    test.done()

  'correctly detects and processes IPv4, IPv4-mapped, and IPv6 CIDR addresses': (test) ->
    cidr = ipaddr.processCIDR('1.2.3.4/5')
    test.deepEqual([ipaddr.process('0.0.0.0'), 5], cidr)
    test.equal(cidr[0].kind(), 'ipv4')

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1/23')
    test.deepEqual([ipaddr.process('::'), 23], cidr)
    test.equal(cidr[0].kind(), 'ipv6')

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1/64')
    test.deepEqual([ipaddr.process('::'), 64], cidr)
    test.equal(cidr[0].kind(), 'ipv6')

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1/96')
    test.deepEqual([ipaddr.process('0.0.0.0'), 0], cidr)
    test.equal(cidr[0].kind(), 'ipv4')

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1/98')
    test.deepEqual([ipaddr.process('192.0.0.0'), 2], cidr)
    test.equal(cidr[0].kind(), 'ipv4')

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1/128')
    test.deepEqual([ipaddr.process('192.168.1.1'), 32], cidr)
    test.equal(cidr[0].kind(), 'ipv4')

    cidr = ipaddr.processCIDR('fc00::/64')
    test.deepEqual([ipaddr.process('fc00::'), 64], cidr)
    test.equal(cidr[0].kind(), 'ipv6')

    cidr = ipaddr.processCIDR('2600:10e5::af/123')
    test.deepEqual([ipaddr.process('2600:10e5::a0'), 123], cidr)
    test.equal(cidr[0].kind(), 'ipv6')

    # Now verify with zone indexes

    cidr = ipaddr.processCIDR('::ffff:192.168.1.1%z/98')
    test.deepEqual([ipaddr.process('192.0.0.0'), 2], cidr)
    test.equal(cidr[0].kind(), 'ipv4')
    test.equal(cidr[0].zoneId, undefined)

    cidr = ipaddr.processCIDR('fc00::%z/64')
    test.deepEqual([ipaddr.process('fc00::%z'), 64], cidr)
    test.equal(cidr[0].kind(), 'ipv6')
    test.equal(cidr[0].zoneId, 'z')

    cidr = ipaddr.processCIDR('2600:10e5::af%z/123')
    test.deepEqual([ipaddr.process('2600:10e5::a0%z'), 123], cidr)
    test.equal(cidr[0].kind(), 'ipv6')
    test.equal(cidr[0].zoneId, 'z')
